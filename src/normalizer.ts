...
type Rule = (schema: LinkedJSONSchema, fileName: string, options: Options, key: string | null) => void;
...
rules.set('Add an $id to anything that needs it', (schema, fileName, _options, _key) => {
	if (!isSchemaLike(schema)) {
		return;
	}
